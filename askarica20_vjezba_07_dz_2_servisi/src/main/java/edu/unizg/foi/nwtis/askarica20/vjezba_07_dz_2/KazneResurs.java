...
import edu.unizg.foi.nwtis.askarica20.vjezba_07_dz_2.podaci.KaznaDAO;
...
public class KazneResurs extends SviResursi {
  ...
  public Response getJsonKaznaVozilo(@HeaderParam("Accept") String tipOdgovora,
    ...
    if (odVremena <= 0 || doVremena <= 0) {
      ...
      return Response.status(Response.Status.OK)
          .entity(kaznaDAO.dohvatiKazneVozilaUIntervalu(id, odVremena, doVremena)).build();
    }
  }
  ...
  /**
   * Provjerava dostupnost poslužitelja slanjem testne poruke.
   *
   * @return true ako je poslužitelj dostupan, inače false
   */
  private boolean provjeriPosluzitelja() {
    var poruka = new StringBuilder();
    poruka.append("TEST").append("\n");

    // todo trenutno je ukodirana adresa i broj mreznih vratiju - treba preuzeti iz konfiguracijskih
    // podataka


    var odgovor = MrezneOperacije.posaljiZahtjevPosluzitelju(this.adresaKazne,
        this.mreznaVrataKazne, poruka.toString());
